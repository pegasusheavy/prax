...
mod diesel_async_benchmarks {
    ...
    pub fn update_query() -> String {
        use schema::users::dsl::*;
        let query = diesel::update(users.filter(id.eq(1i64)))
            .set((
    ...
    /// Create an AND filter with 5 conditions
    pub fn and_filter_5() -> Box<dyn BoxableExpression<users::table, Pg, SqlType = diesel::sql_types::Bool>> {
        Box::new(
            status.eq("active")
                .and(age.gt(18))
                .and(age.lt(65))
    ...
    /// Create an AND filter with 10 conditions
    pub fn and_filter_10() -> Box<dyn BoxableExpression<users::table, Pg, SqlType = diesel::sql_types::Bool>> {
        Box::new(
            status.eq("active")
                .and(age.gt(18))
                .and(age.lt(65))
    ...
    /// Create an OR filter with 5 conditions
    pub fn or_filter_5() -> Box<dyn BoxableExpression<users::table, Pg, SqlType = diesel::sql_types::Bool>> {
        Box::new(
            role.eq("admin")
                .or(role.eq("moderator"))
                .or(role.eq("editor"))
    ...
    /// Create an IN filter with 100 values
    pub fn in_filter_100() -> Box<dyn BoxableExpression<users::table, Pg, SqlType = diesel::sql_types::Bool>> {
        let values: Vec<i64> = (0..100).collect();
        Box::new(id.eq_any(values))
    }

    /// Create complex nested filter
    pub fn complex_nested_filter() -> Box<dyn BoxableExpression<users::table, Pg, SqlType = diesel::sql_types::Bool>> {
        Box::new(
